...
\While{there is a vertex $r \in V \setminus \mathcal{X}$ with {\color{red}$|B^{out}_G(r, 4D \log n) \cap S| \leq \frac{2}{3}|S|$} \textbf{ or } {\color{red}$|B^{in}_G(r, 4D \log n) \cap S| \leq \frac{2}{3} |S|$}}{
    $\tilde{D} \gets  \textsc{ExpDistr}(D)$.\\
